...
# Отображение логов
Для включения или отключения логов от SDK, нужно указать параметр logging при создании экземпляра класса (По умолчанию логи включены):
```kotlin
    var logging = true
    ...
    IQChannels.configure(
        this, 
        IQChannelsConfig(address, channels, chatToOpen, logging, UIOptions(true)),
    )
```
